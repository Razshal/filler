...
/*   Updated: 2018/01/26 15:24:02 by mfonteni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_grid_size(t_fill *infos)
{
	...
	while (ft_ispace(temp[count]))
		count++;
	infos->gridsize.y = ft_atoi(&temp[count]);
}
...
void	set_piece_size(t_fill *infos)
{
	...
	while (!ft_strstr(infos->currentline, "Piece"))
		get_next_line(0, &infos->currentline);
	infos->piecesize.x = ft_atoi((temp =
	...
	while (ft_ispace(temp[count]))
		count++;
	infos->piecesize.y = ft_atoi(&temp[count]);
}
...
void	grid_parser(t_fill *infos)
{
	...
	line = 0;
	if (infos->gridsize.x == -1)
		set_grid_size(infos);
	if (!(infos->grid = (char**)malloc(sizeof(char*) * infos->gridsize.x + 2)))
		return ;
	while (line < infos->gridsize.x)
	{
		if (!infos->grid[line])
			infos->grid[line] = ft_strnew(infos->gridsize.y + 1);
...
void	piece_parser(t_fill *infos)
{
	...
	if (!(infos->currentpiece
		...
		return ;
	while (line < infos->piecesize.x)
	{
		if (!infos->currentpiece[line])
			infos->currentpiece[line] = ft_strnew(infos->piecesize.y + 1);
